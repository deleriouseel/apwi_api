...
router = APIRouter(prefix="/v1", tags=["Apply Within Programs by Network"])
...
@router.get(
    "/{network}",
    summary="List programs on one network: CALVARY or ACN",
...
def get_network(
    ...
    if search is None:
        programs = (
            ...
            .filter(models.APWI.network == network)
            .filter(models.APWI.airdate <= datetime.date.today().isoformat())
            .order_by(desc(models.APWI.airdate))
            .limit(limit)
            .offset(skip)
            .all()
        )
...
@router.get(
    "/{network}/{start_date}",
    summary="Get programs by network and date",
    response_model=List[schemas.ProgramBase],
)
def get_network_dates(
    network: str,
    response: Response,
    db: Session = Depends(get_db),
    start_date: str = Path(regex="^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$"),
    ...
    programs = (
        db.query(models.APWI)
        .filter(models.APWI.network == network)
        .filter(models.APWI.airdate.between(start_date, end_date))
        .order_by(desc(models.APWI.airdate))
        .all()
    )
